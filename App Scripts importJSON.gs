...
function ImportJSONAdvanced(url) {
  var jsondata = UrlFetchApp.fetch(url);
  var jsonObject = JSON.parse(jsondata.getContentText());
  return processLeadsData(jsonObject);
}
function processLeadsData(jsonObject) {
  var leadsData = [];
  var data = jsonObject.data;

  // Add headers
  var headers = ['created_time', 'id', 'full_name', 'phone_number', 'email'];
  leadsData.push(headers);

  // Define a mapping of Arabic field names to English
  var fieldMapping = {
    'الأسم': 'full_name',
  ...
  data.forEach(function(lead) {
    var rowData = [];
    rowData.push(lead.created_time);
    rowData.push(lead.id);
    ...
    lead.field_data.forEach(function(field) {
      // Check if field.name is in Arabic and map it to the English equivalent
      var fieldName = fieldMapping[field.name] || field.name;
    ...
    rowData.push(fields.full_name);
    rowData.push(fields.phone_number);
    rowData.push(fields.email);

    // Add the row to the leadsData
    leadsData.push(rowData);
  });
...
function importLeads() {
  ...
  var configRange = configSheet.getRange("A2:B" + configSheet.getLastRow());
  ...
  configValues.forEach(function(row) {
    ...
    if (formId && sheetName) {
      ...
      var existingIds = sheet.getRange(2, 2, sheet.getLastRow(), 1).getValues().flat(); // Assuming IDs are in column 2
      
      // Filter out already imported leads
      var newData = data.filter(function(row, index) {
        return index === 0 || !existingIds.includes(row[1]); // Check if ID is not in existingIds
      });
